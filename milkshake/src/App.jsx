...
import Layout from './components/layout/Layout'; // ← importera Layout
import { db } from './firebase';
import { collection, onSnapshot, addDoc, updateDoc, deleteDoc, doc } from 'firebase/firestore';

// Separat komponent så vi kan använda useLocation() inuti Router
function AppContent() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(reviewsCollection, (snapshot) => {
      const firebaseReviews = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
      setReviews(firebaseReviews.sort((a, b) => new Date(b.date) - new Date(a.date)));
  ...
  const handleSave = async () => {
    ...
    const reviewData = { ...formData, place: formattedPlace, location: formattedLocation };
  ...
  const toggleFavorite = (id) => {
    setLocalFavorites(prev => {
      const nextFavs = prev.includes(id) ? prev.filter(f => f !== id) : [...prev, id];
  ...
  return (
    // Layout får showForm/setShowForm och vet om vi är på startsidan
    <Layout showForm={showForm} setShowForm={setShowForm} isHomePage={isHomePage}>
      <Routes>
        <Route path="/" element={
          <main className="container">
            ...
            <div className="stats-grid">
              ...
              <div className="stat-box purple"><strong>{reviews.length > 0 ? (reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length).toFixed(1) : 0}⭐️</strong><br/>Snittbetyg</div>
              <div className="stat-box blue"><strong>{new Set(reviews.map(r => r.place)).size}</strong><br/>Ställen</div>
              <div className="stat-box green"><strong>{reviews.length > 0 ? (reviews.reduce((acc, r) => acc + (parseFloat(r.price) || 0), 0) / reviews.length).toFixed(0) : 0} kr</strong><br/>Snittkostnad</div>
            ...
            {showForm && (
              <div className="form-card">
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '10px' }}>
                    {["Annika", "Mattias"].map(name => (
                      <button key={name} type="button" onClick={() => setFormData({...formData, reviewer: name})}
                ...
                <div style={{ marginBottom: '1.5rem', textAlign: 'center', background: '#fdf2f8', padding: '20px', borderRadius: '16px', border: '1px solid #fbcfe8' }}>
                  <p style={{ fontSize: '0.85rem', color: 'var(--text-dark)', marginBottom: '15px', fontWeight: 'bold' }}>DRA FÖR ATT SÄTTA DITT BETYG :D</p>
                  <Rating value={formData.rating} precision={0.5} readOnly size="large"
                    sx={{ fontSize: '3rem', marginBottom: '12px', color: 'var(--pink-500)',
                      '& .MuiRating-iconFilled': { background: 'linear-gradient(135deg, var(--pink-400), var(--lilac-500))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' },
                      '& .MuiRating-iconEmpty': { color: 'var(--pink-200)', opacity: 0.6 },
                    }}
                  />
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#9333ea', marginBottom: '10px' }}>{formData.rating.toFixed(1)}</div>
                  <div style={{ padding: '0 20px' }}>
                    <Slider value={formData.rating} min={0.5} max={5} step={0.5}
                      onChange={(e, newValue) => setFormData({...formData, rating: newValue})}
                      sx={{ color: 'transparent',
                        '& .MuiSlider-track': { background: 'linear-gradient(90deg, #ffadd6, #ff4da6, #b388ff)', height: 12, borderRadius: 999 },
                        '& .MuiSlider-rail': { background: '#ffe0f0', opacity: 1, height: 12, borderRadius: 999, border: '2px solid #ffadd6' },
                        '& .MuiSlider-thumb': { height: 34, width: 34, backgroundColor: 'white', border: '4px solid #ff80bf', borderRadius: '50%', boxShadow: '4px 4px 0 #ffadd6' },
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="input-row">
                  ...
                  <input type="number" placeholder="Pris (kr)" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} />
                </div>

                <textarea placeholder="Hur smakade den?" rows="3" value={formData.review} onChange={e => setFormData({...formData, review: e.target.value})} />
                  ...
                  {expandedPlace === placeName && (
                    <div className="review-grid" style={{ padding: '1rem 0' }}>
                      {groupedReviews[placeName].map(r => (
                        <ReviewCard key={r.id}
                          review={{ ...r, favorite: localFavorites.includes(r.id) }}
